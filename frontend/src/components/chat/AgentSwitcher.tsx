...
export default function AgentSwitcher({ agents, selected, onSelect }: AgentSwitcherProps) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 rounded-xl border border-zinc-800/80 bg-zinc-900/60 px-3 py-1.5 text-sm font-medium transition-all hover:bg-zinc-800/60 hover:border-zinc-700"
      >
      ...
      {open && (
        <div className="animate-scale-in absolute left-0 top-full z-10 mt-2 w-80 overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/95 backdrop-blur-md shadow-2xl shadow-black/40">
            ...
            {agents.map((a) => (
              <button
                ...
                className={`flex w-full items-start gap-2.5 px-3 py-2.5 text-left transition ${
                  a.slug === selected ? "bg-indigo-500/5" : "hover:bg-zinc-800/60"
                }`}
              >
                <div className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${
                  a.slug === selected ? "bg-indigo-500/15" : "bg-zinc-800/60"
                }`}>
